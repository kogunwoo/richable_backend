...
class OutcomeServiceImplTest {
    ...
    void addOutcome() {
        ...
        outcomeUserDTO.setExpCategory("Grocery");
        outcomeUserDTO.setDate("2024-10-14");
        outcomeUserDTO.setAmount(100L);
        outcomeUserDTO.setMemo("Weekly groceries");
        outcomeUserDTO.setDescript("Bought fruits and vegetables");

        OutcomeUser savedOutcome = new OutcomeUser();
        savedOutcome.setIndex(1);
        savedOutcome.setUid(member);
        savedOutcome.setCategory(outcomeUserDTO.getExpCategory());
        try {
            savedOutcome.setDate(new SimpleDateFormat("yyyy-MM-dd").parse(outcomeUserDTO.getDate()));
        } catch (ParseException e) {
            throw new RuntimeException(e);
        }
        savedOutcome.setAmount(outcomeUserDTO.getAmount());
        savedOutcome.setDescript(outcomeUserDTO.getDescript());
        savedOutcome.setMemo(outcomeUserDTO.getMemo());
